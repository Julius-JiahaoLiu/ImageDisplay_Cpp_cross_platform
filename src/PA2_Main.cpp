...
vector<VectorData> buildVectorSpace(unsigned char *&inData, int width, int height, int blockWidth, int blockHeight, int chanelCnt);

/* K-Means Clustering for one channel */
pair<vector<int>, vector<VectorData>> kMeansClustering(const vector<VectorData> &vectors, int K, int maxIterations);
...
MyFrame::MyFrame(const wxString &title, string imagePath, int vectorSize, int vectorNumber) : wxFrame(NULL, wxID_ANY, title)
{
  ...
  vector<VectorData> vectorSpace = buildVectorSpace(inData, width, height, blockWidth, blockHeight, channelCnt);

  auto end = std::chrono::high_resolution_clock::now(); // End timing
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
  cout << "Time taken for Reading Image Data and Building Vector Space: " << duration << " milliseconds" << endl;
  cout << endl;
  ...
  auto [assignments, centroids] = kMeansClustering(vectorSpace, vectorNumber, 200);
...
vector<VectorData> buildVectorSpace(unsigned char *&inData, int width, int height, int blockWidth, int blockHeight, int channelCnt)
{
  ...
  for (int i = 0; i < height; i += blockHeight)
  {
    for (int j = 0; j < width; j += blockWidth)
    {
      ...
      vectorSpace.push_back({vec}); // Store the vectorData for each pixel block in the image with RGB sequence
    }
  }
  return vectorSpace;
}
...
vector<VectorData> centroidsInitializationUniform(int K, int M, int channelCnt)
{
  /* Step 1: Initialize K centroids uniformly in [0, 255]^M
    ...
    K being a power of 2 allows us to choose a grid size where the total number of points is K.
    For M >= 2, distribute K centroids in a grid-like structure
  */
  vector<VectorData> centroids(K, {vector<vector<double>>(M, vector<double>(channelCnt, 0.0))}); // Initialize K centroids
  int sqrtM = static_cast<int>(sqrt(static_cast<double>(M)));                                    // Since M is 2 or a perfect square
  int divisionsPerDim = static_cast<int>(pow(K, 1.0 / sqrtM) + 0.5);                             // K^(1/sqrt(M)), rounded
  double step = 255.0 / divisionsPerDim;                                                         // Step size for each dimension
  int centroidIdx = 0;
  for (int i = 0; i < divisionsPerDim && centroidIdx < K; i++)
  {
    for (int j = 0; j < divisionsPerDim && centroidIdx < K; j++)
    {
      VectorData &centroid = centroids[centroidIdx];
      // Fill first sqrt(M) dimensions with i-based grid
      for (int m = 0; m < sqrtM && m < M; m++)
      {
        for (int k = 0; k < channelCnt; k++)
        {
          centroid.vec[m][k] = i * step + 0.5 * step; // Center of the grid cell
        }
      }
      // Fill remaining dimensions with j-based grid
      for (int m = sqrtM; m < M; m++)
      {
        for (int k = 0; k < channelCnt; k++)
        {
          centroid.vec[m][k] = j * step + 0.5 * step; // Center of the grid cell
        }
      }
      centroidIdx++;
    }
...
pair<vector<int>, vector<VectorData>> kMeansClustering(const vector<VectorData> &vectors, int K, int maxIterations)
{
  if (vectors.empty() || K <= 0 || K > static_cast<int>(vectors.size()))
  {
    cerr << "Invalid input vectorData for K-means, number of clusters must be less than or equal to input vectorData number." << endl;
    exit(1);
  }
  ...
  return {assignments, centroids}; // Return cluster assignments and centroids
}
